...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  auto look_ahead_distance = VelocityProfileGenerator::calc_distance(velocity_mag, 0.0, -_max_accel);
...
//             .count() << std::endl;
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    if (is_goal_in_junction) {
      ...
      // sin/cos(yaw)을 곱해주는 이유는 도로에서의 제한속도값은 일정한 값이나, 
      // 도로의 yaw값에 따라 x,y축에 프로젝션된 속도값은 제각각 달라지므로 이를 고려하기 위함임.
      goal.velocity.x = _speed_limit * cos(goal.rotation.yaw);  // 목표지점 도로의 제한 속도_x
      goal.velocity.y = _speed_limit * sin(goal.rotation.yaw);  // 목표지점 도로의 제한 속도_y
      goal.velocity.z = 0;
    }
    ...
    if (distance_to_stop_sign <= P_STOP_THRESHOLD_DISTANCE) {
      // 차량의 현재 위치가 정지선과 일정값 이하로 가까이 있다면, 
      // FSM 로직을 DECEL_TO_STOP에서 STOPPED으로 변경
